...
import { annualEvents } from "data/annualEvents";
...
const slides: Slide[] = [
  {
    ...
    title: "Annual Events",
    subtitle: "Annual events happening at UBC",
...
const AnnualEventsPage = () => {
  return (
    <Layout seoTitle="Annual Events">
      ...
      <ContentSection>
        <Box>
          ...
          <SimpleGrid columns={{ base: 1, md: 2 }} spacing="12" mb="5">
            {annualEvents.map(
              ({ eventType, date, media, title, description, page, href }) => (
                <Event
                  eventType={eventType}
                  date={date}
                  media={media ?? ""}
                  ...
                  href={`/annual-events/${toKebabCase(title)}`}
                  noTruncate
                />
              )
            )}
      ...
      <FullWidthCTA
        title="Calendar"
        text="View all events happening at or around the CUS"
